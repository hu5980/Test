...
- (void)setDefaults {
    ...
    self.selectionIndicatorPosition = HMSelectionIndicatorTop;
...
//    [self.textColor set];
    [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
        ...
        //fanzhanao@gmail.com add begin
        UILabel *lbl = [[UILabel alloc] initWithFrame:rect];
        [lbl setText:titleString];
        if (idx == self.selectedIndex) {
            [lbl setTextColor:self.textSelectedColor];
        }
        else {
        ...
        if (idx < [self.sectionTitles count] -1) {
            UIView *sep = [[UIView alloc] initWithFrame:CGRectMake(rect.origin.x+rect.size.width-self.sepViewWidth/2,rect.origin.y,self.sepViewWidth,rect.size.height)];
            [sep setBackgroundColor:self.sepColor];
            [self addSubview:sep];
        }
        //fanzhanao@gmail.com add end
...
- (CGRect)frameForSelectionIndicator {
    ...
    float selectHeight = 0.0;
    if (self.selectionIndicatorPosition == HMSelectionIndicatorBottom) {
        selectHeight = self.frame.size.height - self.selectionIndicatorHeight;
    }
    if (self.selectionIndicatorMode == HMSelectionIndicatorResizesToStringWidth) {
        ...
        return CGRectMake(x, selectHeight, stringWidth, self.selectionIndicatorHeight);
    } else {
        return CGRectMake(self.segmentWidth * self.selectedIndex, selectHeight, self.segmentWidth, self.selectionIndicatorHeight);
    }
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        //fanzhanao@gmail.com add begin
        NSUInteger i = 0;
        for (UIView *lbl_viw in [self subviews]) {
            if ([lbl_viw isKindOfClass:[UILabel class]]) {
                if (i == segment) {
                    [(UILabel*)lbl_viw setTextColor:self.textSelectedColor];
                }else{
                    [(UILabel*)lbl_viw setTextColor:self.textColor];
                }
                ++i;
            }
        }
